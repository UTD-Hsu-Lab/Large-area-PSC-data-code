...
set n @node@
# the incidence power from optical sim is sum of intensity in the spectrum file.
set Psolar [expr 100]	;# Intensity [mW/cm2] of am1.5g spectrum
set Xlen @xlen@
set mobility @transport_mobility@

set cntlen @cntlen@
set areafactor [expr 1e12/($Xlen*(1+2*$cntlen))]
echo "af = $areafactor \[cm^2\]"
load_file n@node|sdevice@_des.plt -name Plt_IV
...
#------------------------------------------------------------
	...
	#set label "Xlen\tYlen\tST\tSW\tSLX\tSLY\tRsh\tRs\tVoc\tJsc\tVmpp\tJmpp\tPmpp\tFF\tEff"

	puts  $fp "JV-Curve\n $mobility size $Xlen" 
	puts  $fp $Vs 
	puts  $fp $Js 

export_curves {j} -plot PlotIV -filename @pwd@/n${n}_iv_original.csv -format csv -overwrite
